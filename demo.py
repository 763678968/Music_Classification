...
train_x, test_x, train_y, test_y = train_test_split(voices, labels,
                                                    test_size=0.1)
...
def train_neural_network():
    ...
    cost = tf.reduce_mean(
        tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=Y, logits=output)))
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(200):
            ...
            for banch in range(n_banch):
                voice_banch = train_x[
                              banch * banch_size:(banch + 1) * (banch_size)]
                label_banch = train_y[
                              banch * banch_size:(banch + 1) * (banch_size)]
                _, loss = sess.run([train_step, cost],
                                   feed_dict={X: voice_banch, Y: label_banch})
